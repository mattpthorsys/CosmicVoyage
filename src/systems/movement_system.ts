...
export class MovementSystem {
  ...
  constructor(player: Player) {
    ...
    this.unsubscribeMoveRequest = eventManager.subscribe(GameEvents.MOVE_REQUESTED, (data) => {
      this.handleMoveRequest(data);
    });
  ...
  handleMoveRequest(data: MoveRequestData): void {
    ...
    logger.debug(
      `[MovementSystem] Handling move request in context: ${data.context} (dx: ${data.dx}, dy: ${data.dy}, SpeedMult: ${speedMultiplier.toFixed(3)})`
    );
  ...
  private _moveWorld(
    ...
    logger.debug(
      `[MovementSystem] Player moved HYPERSPACE: [${oldX},${oldY}] -> [${position.worldX},${position.worldY}] (Delta: ${dx},${dy})`
    );
  }
  ...
  private _moveSystem(
    ...
    logger.debug(
      `[MovementSystem] Player moved SYSTEM: [${oldX.toExponential(2)},${oldY.toExponential(2)}] -> [${position.systemX.toExponential(2)},${position.systemY.toExponential(2)}] (Scale: ${moveScale.toExponential(1)} m/tick)`
    );
    if (oldShipDirection !== render.directionGlyph) {
      logger.debug(
        `[MovementSystem] Player direction changed: ${oldShipDirection} -> ${render.directionGlyph}`
      );
    }
  }
