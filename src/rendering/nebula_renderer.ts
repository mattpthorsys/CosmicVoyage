...
export class NebulaRenderer {
  ...
  prefetchRegion(startWorldX: number, startWorldY: number, cols: number, rows: number, margin = 0): void {
    const availableCacheSlots = this.maxNebulaCacheSize - this.nebulaCacheSize - this.pendingPrefetchKeys.size;
    ...
    void this.provider.getBackgroundColorsAsync(requests).then((samples) => {
