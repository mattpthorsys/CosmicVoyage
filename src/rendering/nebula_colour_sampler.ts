...
export interface NebulaColourRequest {
  worldX: number;
  worldY: number;
}

export interface NebulaColourSample extends NebulaColourRequest {
  colour: string;
}

/** Deterministic nebula colour maths, isolated so workers and renderers share the same source. */
export class NebulaColourSampler {
  ...
  private sampleNebulaColour(region: NebulaRegion, cloud: number, wisps: number): RgbColour {
    ...
    const base = this.mixWeighted(darkReflection, combinedWeight, emission, region.emissionWeight);
