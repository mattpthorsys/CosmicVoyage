...
export class Game {
    ...
    constructor(
        ...
        logger.info(`Game instance constructed successfully. Seed: "${this.gameSeedPRNG.getInitialSeed()}", Initial State: '${this.state}'`); //
    }
    ...
    private _queueActionFromKey(key: string, isShiftDown: boolean): void {
        ...
        switch (key) {
            ...
            case bindings.DOWNLOAD_LOG: action = 'DOWNLOAD_LOG'; break; // <<< Handle new binding
    ...
    private _handleInput(): void {
        ...
        try {
            switch (this.state) {
                ...
                default: logger.warn(`No input handler for unknown state: ${this.state}`); break;
            }
        } catch (inputError) {
            ...
            if (planet) { // Check the local variable
                 ...
                 try {
                    ...
                    this.player.moveSurface(dx, dy, mapSize); // Player method logs details
                     ...
                     this.statusMessage = `Surface Error: ${surfaceError instanceof Error ? surfaceError.message : String(surfaceError)}`;
                 }
            } else {
    ...
    private _handleInputStarbase(action: string): void {
        switch (action) {
            case 'LIFTOFF': this._liftoffAction(); break; // Use same liftoff logic
    ...
    private _update(deltaTime: number): void { // Keep deltaTime here, used by system updates
        // logger.debug(`Updating state: ${this.state}`); // Noisy
        try {
             switch (this.state) {
                 case 'hyperspace': this._updateHyperspace(deltaTime); break; //
                 case 'system': this._updateSystem(deltaTime); break; // Pass deltaTime
    ...
    private _updateHyperspace(_deltaTime: number): void { // Ignore TS6133 if it appears, rename if truly never needed
    ...
    private _updateSystem(deltaTime: number): void { // Keep deltaTime, it's used
        ...
        this.currentSystem.planets.forEach(p => {
            if(p) {
                ...
                if (dSq < currentMinDistSq) { // Use < to find the closest one
        ...
        if(this.currentSystem.starbase) {
            ...
            if (dSq < currentMinDistSq) { // Check if closer than closest planet
        ...
        if (nearbyObject) {
             ...
             status += ` | Near ${nearbyObject.name} (${dist.toFixed(0)} units). Press [${CONFIG.KEY_BINDINGS.LAND}] to approach/land.`;
        }
    ...
    private _landAction(): void {
        ...
        if(this.currentSystem.starbase) {
            const dSq = this.player.distanceSqToSystemCoords(this.currentSystem.starbase.systemX, this.currentSystem.starbase.systemY);
            if (dSq < currentMinDistSq) { nearbyObject = this.currentSystem.starbase; }
        }
        ...
        if (nearbyObject) {
             ...
             try {
                  ...
                  if (nearbyObject instanceof Planet) {
                       this.currentPlanet = nearbyObject;
                       ...
                       logger.info(`Game state changed: '${oldState}' -> '${this.state}' (Landed on planet ${this.currentPlanet.name})`);
                  } else if (nearbyObject instanceof Starbase) {
                       this.currentStarbase = nearbyObject;
                       ...
                       logger.info(`Game state changed: '${oldState}' -> '${this.state}' (Docked at starbase ${this.currentStarbase.name})`);
                  }
             } catch (error) {
    ...
    private _scanPlanetAction(): void {
        ...
        try {
             ...
             this.statusMessage = `${this.currentPlanet.name} scan complete. Richness: ${this.currentPlanet.mineralRichness}, Resource: ${this.currentPlanet.primaryResource || 'N/A'}.`;
             logger.info(`Scan successful. Richness: ${this.currentPlanet.mineralRichness}, Resource: ${this.currentPlanet.primaryResource}`);
        } catch (error) {
    ...
    private _refuelAction(): void {
         ...
         logger.info(`Attempting to buy ${fuelToBuy.toFixed(0)} fuel for ${cost} credits at ${this.currentStarbase.name}...`);
         const oldCredits = this.player.credits;
         ...
         if (this.player.fuel >= this.player.maxFuel) {
              this.statusMessage = `Purchased ${fuelToBuy.toFixed(0)} fuel for ${cost} credits. Fuel tank full! (${this.player.fuel.toFixed(0)}/${this.player.maxFuel})`;
         } else {
              this.statusMessage = `Purchased ${fuelToBuy.toFixed(0)} fuel for ${cost} credits. (${this.player.fuel.toFixed(0)}/${this.player.maxFuel})`;
         }
     }
