...
export function createAvailableActions(context: AvailableActionContext): AvailableAction[] {
  const actions: AvailableAction[] = [
    action(
      'primary',
      'Do Best Action',
      CONFIG.KEY_BINDINGS.PRIMARY_ACTION,
      'PRIMARY_ACTION',
      'utility',
      0,
      true
  ...
  if (context.state !== 'starbase' && context.state !== 'orbit') {
    actions.push(
      action('ship-menu', 'Ship Menu', CONFIG.KEY_BINDINGS.SHIP_MENU, 'SHIP_MENU', 'utility', 8, true)
    );
  }
    ...
    case 'hyperspace':
      if (context.isNearHyperspaceSystem) {
        ...
        actions.push(
          action(
            'scan-system',
            'Scan System',
            CONFIG.KEY_BINDINGS.SCAN_SYSTEM_OBJECT,
            'SCAN_SYSTEM_OBJECT',
            'target',
            20,
            true,
      ...
      actions.push(
        action(
          'fine-control',
          'Fine Drift',
          CONFIG.KEY_BINDINGS.FINE_CONTROL,
          'FINE_CONTROL',
          'movement',
          83,
          true
      ...
      actions.push(
        action('diagonal-drift', 'Diagonal Drift', 'Numpad 1/3/7/9', 'MOVE_DIAGONAL', 'movement', 84, true)
      );
      break;
    case 'system':
      actions.push(
        action(
          'cycle-target',
          'Cycle Target',
          CONFIG.KEY_BINDINGS.CYCLE_TARGET,
          'CYCLE_TARGET',
          'target',
          6,
          true,
          context.selectedTargetName ?? undefined
        )
      );
      actions.push(
        action(
          'target-menu',
          'Target Menu',
          CONFIG.KEY_BINDINGS.TARGET_MENU,
          'TARGET_MENU',
          'target',
          7,
          true,
      ...
      if (context.hasSelectedTarget) {
        actions.push(
          action(
            'approach-target',
            'Approach Target',
            CONFIG.KEY_BINDINGS.APPROACH_TARGET,
            'APPROACH_TARGET',
            'navigation',
            35,
            true,
      ...
      if (context.nearbyObject) {
        ...
        actions.push(
          action(
            'land-dock',
            label,
            CONFIG.KEY_BINDINGS.ACTIVATE_LAND_LIFTOFF,
            'ACTIVATE_LAND_LIFTOFF',
            'navigation',
            10,
            true,
            context.nearbyObject.name
          )
        );
        actions.push(
          action(
      ...
      if (context.isNearSystemEdge) {
        actions.push(
          action(
            'leave-system',
            'Leave System',
            CONFIG.KEY_BINDINGS.LEAVE_SYSTEM,
            'LEAVE_SYSTEM',
            'navigation',
            15,
            true
      ...
      actions.push(
        action('zoom-out', 'Zoom Out', CONFIG.KEY_BINDINGS.ZOOM_OUT, 'ZOOM_OUT', 'utility', 81, true)
      );
      break;
    case 'orbit':
      actions.push(
        action('select-body', 'Select Body', 'Left/Right', 'MOVE', 'target', 6, true, context.planet?.name)
      );
      actions.push(
        action(
          'landing-site',
    ...
    case 'planet':
      if (context.planet && context.planet.type !== 'GasGiant' && context.planet.type !== 'IceGiant') {
        if (!context.planet.scanned) {
          ...
          actions.push(
            action('mine', 'Mine', CONFIG.KEY_BINDINGS.MINE, 'MINE', 'surface', 20, true, context.planet.name)
          );
        }
      }
      break;
    case 'starbase':
      ...
      actions.push(
        action(
          'section-right',
          'Next Section',
          CONFIG.KEY_BINDINGS.MOVE_RIGHT,
          'MOVE_RIGHT',
          'navigation',
          7,
          true
  ...
  actions.push(
    action(
      'profiler',
      'Profiler',
      CONFIG.KEY_BINDINGS.TOGGLE_PROFILER,
      'TOGGLE_PROFILER',
      'utility',
      96,
      true
...
/** Creates one contextual action descriptor. */
function action(
  id: string,
  label: string,
  key: string,
