...
export class HeightmapGenerator {
  ...
  /** Initializes HeightmapGenerator. */
  constructor(targetSize: number, roughness: number, seed: string) {
    logger.debug(
      `[HeightmapGen] Initializing for target size ${targetSize}, roughness ${roughness}, seed "${seed}"...`
    );
  ...
  private get(x: number, y: number): number {
    ...
    try {
      ...
      logger.error(
        `[HeightmapGen.get] Error accessing map at wrapped coords [${wrappedX}, ${wrappedY}] (original: [${x}, ${y}]). Size: ${this.size}`,
        e
      );
  ...
  private set(x: number, y: number, val: number): void {
    ...
    if (intX >= 0 && intX <= this.max && intY >= 0 && intY <= this.max) {
      ...
      try {
        ...
        logger.error(
          `[HeightmapGen.set] Error setting map at valid coords [${intX}, ${intY}]. Size: ${this.size}`,
          e
        );
      }
    } else {
      logger.warn(
        `[HeightmapGen.set] Attempted to set value out of bounds: [${intX}, ${intY}] (Max index: ${this.max})`
      );
    }
  }
  ...
  generate(initialValueRange: number = 128): number[][] {
    logger.info(
      `[HeightmapGen] Starting Diamond-Square generation... (Initial Range: +/- ${initialValueRange})`
    );
  ...
  private divide(step: number, range: number): void {
    ...
    if (half < 1) {
      // Base case: Stop when step size is too small
    ...
    for (let y = 0; y <= this.max; y += half) {
      // Iterate through all potential square center rows
  ...
  /** Calculates diamond-step midpoint values from four corner samples. */
  ...
  /** Calculates square-step midpoint values from four adjacent diamond samples. */
  ...
  private normalize(): void {
    logger.info(
      `[HeightmapGen] Normalizing heightmap values to range [0, ${CONFIG.PLANET_HEIGHT_LEVELS - 1}]...`
    );
    let min = Infinity,
      max = -Infinity; // Initialize min/max
    ...
    if (range === 0 || !Number.isFinite(range)) {
      // Also check if range is NaN/Infinity
      logger.warn(
        `[HeightmapGen.normalize] Map range is zero or invalid (${range}). Setting all values to midpoint.`
      );
  ...
  /** Copies edge values so the generated heightmap wraps continuously at longitude. */
