...
describe('planet axial tilt generation', () => {
  it('usually generates modest obliquities but includes rare Uranus-like outliers', () => {
    const tilts = Array.from({ length: 400 }, (_, index) => (generateAxialTiltRad(new PRNG(`tilt-${index}`)) * 180) / Math.PI);
  ...
  it('keeps tidally locked bodies at low spin obliquity', () => {
    const tilts = Array.from({ length: 100 }, (_, index) => (generateAxialTiltRad(new PRNG(`locked-tilt-${index}`), true) * 180) / Math.PI);
