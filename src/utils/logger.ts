    ...
    switch (configLevelString) {
      ...
      default:
        if (defaultLevel >= LogLevel.WARN) {
          console.warn(
            `[Logger Init WARN] Invalid LOG_LEVEL in CONFIG: "${CONFIG?.LOG_LEVEL}". Defaulting to ${LogLevel[defaultLevel]}.`
          );
        }
        return defaultLevel;
    }
...
// Writes one already-formatted message to the selected console level.
/** Writes a formatted log entry and stores it in the in-memory buffer. */
...
/** Formats arbitrary logging arguments into readable text. */
...
export const logger: Logger = {
  ...
  debug(...args: LogDataype[]): void {
    if (currentLogLevel >= LogLevel.DEBUG) {
      // Format structured values before forwarding them to the console sink.
  ...
  getLogBufferAsString(includeHeader: boolean = true): string {
    // Default includeHeader to true
    let logContent = '';
    if (includeHeader) {
...
/** Converts a proposed download name into a filesystem-safe filename. */
