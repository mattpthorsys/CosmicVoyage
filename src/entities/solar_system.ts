...
import { SPECTRAL_DISTRIBUTION, SPECTRAL_TYPES, GRAVITATIONAL_CONSTANT_G, AU_IN_METERS, GLYPHS, PLANET_TYPES } from '../constants';
...
export class SolarSystem {
    ...
    constructor(starX: number, starY: number, gameSeedPRNG: PRNG) {
        ...
        if (this.starbase) {
            ...
            logger.info(`[System:${this.name}] Starbase generated at orbit distance ${this.starbase.orbitDistance.toExponential(2)}m.`);
        }
        ...
        logger.debug(`[System:${this.name}] System edge radius calculated: ${this.edgeRadius.toExponential(2)}m (Factor: ${CONFIG.SYSTEM_EDGE_RADIUS_FACTOR})`);
    }
    ...
    private generatePlanets(): void {
        ...
        for (let i = 0; i < CONFIG.MAX_PLANETS_PER_SYSTEM; i++) {
          ...
          let currentOrbitDistance = lastOrbitDistance * Math.pow(orbitScaleBase, 1 + this.systemPRNG.random(-0.2, 0.2))
          ...
          logger.debug(`[System:${this.name}] Slot ${i + 1}: Calculated potential orbit distance ${currentOrbitDistance.toExponential(2)}m (last was ${lastOrbitDistance.toExponential(2)}m)`);

          // Check for starbase collision (using meter-based distance)
          if (this.starbase && Math.abs(currentOrbitDistance - this.starbase.orbitDistance) < MIN_PLANET_SEPARATION_M * 0.5) {
            const oldOrbit = currentOrbitDistance;
            currentOrbitDistance = this.starbase.orbitDistance + MIN_PLANET_SEPARATION_M * 0.6 * (currentOrbitDistance > this.starbase.orbitDistance ? 1 : -1);
            currentOrbitDistance = Math.max(lastOrbitDistance + MIN_PLANET_SEPARATION_M, currentOrbitDistance);
            currentOrbitDistance = Math.min(MIN_OUTER_ORBIT_M, currentOrbitDistance);
            logger.debug(`[System:${this.name}] Slot ${i + 1}: Adjusted orbit from ${oldOrbit.toExponential(2)}m to ${currentOrbitDistance.toExponential(2)}m to avoid starbase.`);
          }
            ...
            if (canHaveMoons && planet.mass && planet.mass > 0 && planet.diameter && planet.diameter > 0) {
                ...
                for (let j = 0; j < maxMoons; j++) {
                    if (moonPRNG.random() < (0.8 / (j + 1))) {
                        ...
                        if (moonOrbit_m > planet.orbitDistance * 0.01) { // Simple check: moon orbit < 1% of planet orbit
                            logger.debug(`[Planet:${planet.name}] Moon orbit ${moonOrbit_m.toExponential(1)}m too large relative to planet orbit. Stopping moon gen.`);
                            break;
                        }
                        ...
                        // Create moon as a Planet instance - requires Planet constructor and generators to handle small sizes
                        try {
                            const moon = new Planet(moonName, moonType, moonOrbit_m, moonAngle, moonPRNG, parentStarType);
                            // Check if moon generation was reasonable (e.g., diameter is smaller than planet)
                            if (moon.diameter * 1000 > planet.diameter * 1000 * 0.5) {
                                logger.warn(`[Planet:${planet.name}] Generated moon ${moonName} is too large (${moon.diameter}km) compared to planet (${planet.diameter}km). Skipping.`);
                                continue;
                            }
                            ...
                            lastMoonOrbit_m = moonOrbit_m;
                             ...
                             logger.error(`[Planet:${planet.name}] Error generating moon ${moonName}: ${moonError}`);
                        }
                    }
                }
            }
    ...
    private getRomanNumeral(num: number): string {
         ...
         const romanMap: Record<number, string> = { 1: 'I', 2: 'II', 3: 'III', 4: 'IV', 5: 'V', 6: 'VI', 7: 'VII', 8: 'VIII', 9: 'IX', 10: 'X', 11: 'XI', 12: 'XII', 13: 'XIII', 14: 'XIV', 15: 'XV', 16: 'XVI', 17: 'XVII', 18: 'XVIII', 19: 'XIX', 20: 'XX' };
    ...
    private determinePlanetType(orbitDistance_m: number): string {
        ...
        if (!starInfo || !starInfo.radius || !starInfo.temp) { logger.warn(`[System:${this.name}] Unknown star type '${this.starType}' or missing radius/temp data. Defaulting to G type properties.`); }
        ...
        if (!Number.isFinite(effectiveTemp)) {
            logger.error(`[System:${this.name}] Calculated non-finite effective temperature (${effectiveTemp}). Defaulting type.`);
            return 'Rock';
        }
        logger.debug(`[System:${this.name}] Effective temp at orbit ${orbitDistance_AU.toFixed(2)} AU: ${effectiveTemp.toFixed(1)}K (RelLum: ${relativeLuminosity.toFixed(2)})`);

        const innerHabitable = 260, outerHabitable = 390, frostLineApprox = 150, hotZone = 800;
        ...
        else if (effectiveTemp > innerHabitable) chosenType = typePRNG.choice(['Rock', 'Oceanic', 'Oceanic', 'Rock', 'Lunar'])!;
        else if (effectiveTemp > frostLineApprox) chosenType = typePRNG.choice(['Rock', 'Frozen', 'GasGiant', 'IceGiant', 'Lunar'])!;
        else chosenType = typePRNG.choice(['GasGiant', 'IceGiant', 'Frozen', 'Frozen', 'Lunar'])!;
        logger.debug(`[System:${this.name}] Determined planet type: ${chosenType} for orbit ${orbitDistance_AU.toFixed(2)} AU.`);
        return chosenType;
    }
    ...
    getObjectNear(x_m: number, y_m: number): Planet | Starbase | null {
        ...
        if(closestObject) {
            logger.debug(`[System:${this.name}] Found nearby object: ${closestObject.name} (DistSq: ${minDistanceSq_m2.toExponential(1)} m^2)`);
        } else {
             logger.debug(`[System:${this.name}] No object found within landing distance (${checkRadius_m} m) of [${x_m.toExponential(1)}, ${y_m.toExponential(1)}]`);
        }
    ...
    updateOrbits(deltaTime: number): void {
        ...
        if (!starMassKg || starMassKg <= 0) { logger.error(`[System:${this.name}] Cannot update orbits: Invalid star mass.`); return; }
        ...
        this.planets.forEach((planet) => {
          ...
          if (!Number.isFinite(planet_r) || planet_r <= 0) { logger.warn(`[System:${this.name}] Invalid orbit distance for ${planet.name}. Skipping.`); return; }
          ...
          if (!Number.isFinite(planetX_abs) || !Number.isFinite(planetY_abs)) { logger.error(`[System:${this.name}] Non-finite position for ${planet.name}. Resetting.`); planet.systemX = 0; planet.systemY = 0; return; }
          ...
          if (planet.moons && planet.moons.length > 0 && planet.mass && planet.mass > 0) {
            planet.moons.forEach((moon) => {
              ...
              if (!Number.isFinite(moon_r_rel) || moon_r_rel <= 0) { logger.warn(`[System:${this.name}] Invalid orbit distance for moon ${moon.name}. Skipping.`); moon.systemX = planet.systemX; moon.systemY = planet.systemY; return; }
              ...
              if (denominator <= 0) { logger.warn(`[System:${this.name}] Invalid denominator for moon period calc for ${moon.name}. Skipping.`); return; }
              const moonPeriod_s = Math.sqrt(numerator / denominator);

              if (!Number.isFinite(moonPeriod_s) || moonPeriod_s <= 0) { logger.warn(`[System:${this.name}] Invalid orbital period (${moonPeriod_s}s) for moon ${moon.name}. Skipping.`); moon.systemX = planet.systemX; moon.systemY = planet.systemY; return; }
              ...
              if (!Number.isFinite(moon.systemX) || !Number.isFinite(moon.systemY)) { logger.error(`[System:${this.name}] Non-finite position for moon ${moon.name}. Resetting.`); moon.systemX = planet.systemX; moon.systemY = planet.systemY; }
            });
          }
        });
        ...
        if (this.starbase) {
          ...
          if (!Number.isFinite(sb_r) || sb_r <= 0) { logger.warn(`[System:${this.name}] Invalid orbit distance for starbase. Skipping.`); return; }
          ...
          if (!Number.isFinite(this.starbase.systemX) || !Number.isFinite(this.starbase.systemY)) { logger.error(`[System:${this.name}] Non-finite position for starbase. Resetting.`); this.starbase.systemX = 0; this.starbase.systemY = 0; }
        }
