...
export function generatePlanetCharacteristics(
  ...
  logger.info(
    `[CharGen] Generating characteristics for Type: ${planetType}, Orbit: ${orbitDistance.toExponential(2)}m, Star: ${parentStarType}, Age: ${environment.ageGyr} Gyr, [Fe/H]: ${environment.metallicityFeH}...`
  );
  ...
  logger.info(
    `[CharGen] Characteristics generated for ${planetType}. Gravity: ${gravity.toFixed(2)}g, EscapeVel: ${escapeVelocity.toFixed(0)} m/s, Richness: ${mineralRichness}.`
  );
...
/** Generates rotation period hours. */
