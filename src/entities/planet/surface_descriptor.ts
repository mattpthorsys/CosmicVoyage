...
export function generateHydrosphere(
  ...
  logger.debug(
    `[SurfDesc] Generating hydrosphere (Temp: ${surfaceTemp}K, Pressure: ${atmosphere.pressure.toFixed(3)} bar)...`
  );
  ...
  if (planetType === 'Molten')
    return pressure > 20
      ? 'Rock vapour and supercritical silicate volatiles'
      : 'None; volatiles thermally stripped';
  ...
  if (planetType === 'Cryovolcanic')
    return getCryovolcanicHydrosphere(tempK, atmosphere, context, volatileScore);
  ...
  if (planetType === 'Lunar') {
    ...
    return volatileScore > 0.22
      ? 'Trace polar ice in permanent shadow'
      : 'None; regolith largely devolatilised';
  }
  ...
  if (volatileScore < 0.18 || (activity > 3.3 && pressure < 0.3)) {
    description =
      tempK > 360
        ? 'None; surface volatiles photolysed or thermally lost'
        : 'Trace adsorbed ice in sheltered regolith';
  } else if (liquidWater && volatileScore > 0.72) {
    description = prng.choice([
      'Significant Saline Oceans and Seas',
      'Connected shallow seas with polar ice caps',
    ])!;
    ...
    description =
      pressure > 0.6
        ? 'Methane/Ethane Lakes over water-ice bedrock'
        : 'Patchy methane frost with transient hydrocarbon pools';
  } else if (nitrogen) {
    description = 'Nitrogen frost plains with transient cryogenic basins';
  } else if (tempK < 245 && volatileScore > 0.35) {
    description =
      volatileScore > 0.65
        ? 'Global Ice Sheet with buried brine reservoirs'
...
/** Estimates surface volatile retention. */
export function estimateSurfaceVolatileRetention(
  ...
  const coolStarRetention =
    spectralClass === 'M' || spectralClass === 'K'
      ? 1.08
      : spectralClass === 'O' || spectralClass === 'B' || spectralClass === 'A'
        ? 0.78
        : 1;
  const coldTrapRetention =
    isColdIcyBody && surfaceTemp < 170 ? 1.45 : isColdIcyBody && surfaceTemp < 245 ? 1.18 : 1;
          ...
          : planetType === 'DwarfIce'
            ...
            : planetType === 'Greenhouse'
              ...
              : planetType === 'CarbonRich'
                ? 0.35
                : planetType === 'Lunar' || planetType === 'Chthonian' || planetType === 'Molten'
                  ? 0.16
                  : 1;
...
/** Returns greenhouse hydrosphere. */
...
/** Returns carbon rich hydrosphere. */
function getCarbonRichHydrosphere(tempK: number, pressure: number, volatileScore: number): string {
  ...
  if (tempK >= 90 && tempK <= 190 && pressure > 0.1)
    return 'Hydrocarbon tars and methane/ethane basin deposits';
  if (tempK < 240) return 'CO/CH4 frosts over dry carbonaceous ice-rock';
  return 'Hydrated minerals only; free water oxygen-starved';
}

/** Returns cryovolcanic hydrosphere. */
function getCryovolcanicHydrosphere(
  tempK: number,
  atmosphere: Atmosphere,
  context: HydrosphereContext,
  ...
  if (
    isHydrocarbonLiquidCandidate(tempK - 22, tempK + 24, atmosphere, atmosphere.pressure, volatileScore, 0.16)
  )
    return 'Methane-rich cryovolcanic lakes over fractured ice';
  if (isNitrogenLiquidCandidate(tempK - 18, tempK + 20, atmosphere, atmosphere.pressure, volatileScore, 0.16))
    return 'Nitrogen-methane cryogenic basins on fractured ice';
  if (tempK >= 170 && tempK <= 265 && volatileScore > 0.24)
    return 'Ammonia-water cryolava reservoirs and unstable surface slush';
...
function getDwarfIceHydrosphere(tempK: number, atmosphere: Atmosphere, volatileScore: number): string {
  ...
  if (isNitrogenLiquidCandidate(tempK - 15, tempK + 18, atmosphere, atmosphere.pressure, volatileScore, 0.12))
    return 'Nitrogen frost plains with transient cryogenic basins';
  if (
    isHydrocarbonLiquidCandidate(tempK - 18, tempK + 24, atmosphere, atmosphere.pressure, volatileScore, 0.12)
  )
    return 'Methane/Ethane Lakes over water-ice bedrock';
  if (tempK >= 55 && tempK <= 125 && volatileScore > 0.16)
    return 'Patchy Nitrogen/Methane Frosts over Ice-Rock';
...
/** Returns frozen hydrosphere. */
function getFrozenHydrosphere(
  tempK: number,
  atmosphere: Atmosphere,
  context: HydrosphereContext,
  volatileScore: number
  ...
  if (
    isHydrocarbonLiquidCandidate(tempK - 22, tempK + 26, atmosphere, atmosphere.pressure, volatileScore, 0.14)
  )
    return 'Methane/Ethane Lakes over water-ice bedrock';
  if (isNitrogenLiquidCandidate(tempK - 18, tempK + 22, atmosphere, atmosphere.pressure, volatileScore, 0.14))
    return 'Nitrogen frost plains with transient cryogenic basins';
  if (tempK > 273 && atmosphere.pressure > 0.01) return 'Cold brine seas amid retreating ice cover';
  if (tempK >= 240 && tempK <= 278 && volatileScore > 0.22)
    return 'Perchlorate brines and widespread ground ice';
  const internalWarmth =
    (context.densityGcm3 ?? 2.8) > 2.35 || (context.environment?.ageGyr ?? 4.6) < 4.2 || volatileScore > 0.38;
...
function isHydrocarbonLiquidCandidate(
  minTemp: number,
  maxTemp: number,
  atmosphere: Atmosphere,
  pressure: number,
  volatileScore: number,
  minVolatileScore: number = 0.3
): boolean {
  const methane = (atmosphere.composition['Methane'] || 0) + (atmosphere.composition['Ethane'] || 0);
  const pressureOk =
    pressure > 0.0005 ||
    atmosphere.density === 'Thin' ||
    atmosphere.density === 'Earth-like' ||
    atmosphere.density === 'Thick';
  return volatileScore > minVolatileScore && methane > 2.5 && pressureOk && minTemp <= 190 && maxTemp >= 88;
}

/** Returns whether nitrogen liquid candidate. */
function isNitrogenLiquidCandidate(
  minTemp: number,
  maxTemp: number,
  atmosphere: Atmosphere,
  pressure: number,
  volatileScore: number,
  minVolatileScore: number = 0.28
): boolean {
  const nitrogen = atmosphere.composition['Nitrogen'] || 0;
  return (
    volatileScore > minVolatileScore && nitrogen > 12 && pressure > 0.0005 && minTemp <= 96 && maxTemp >= 63
  );
}
...
export function generateLithosphere(prng: PRNG, planetType: string): string {
  ...
  switch (planetType) {
    case 'Molten':
      description = 'Silicate Lava Flows, Rapidly Cooling Crust';
      break;
    case 'Rock':
      description = prng.choice([
      ...
      break;
    case 'Oceanic':
      description = 'Submerged Silicate Crust, Probable Hydrothermal Vents';
      break;
    case 'Hycean':
      description = 'Global ocean over high-pressure ice and silicate mantle';
      break;
    case 'Greenhouse':
      description = 'Basaltic highlands, tessera terrain, sulfur-bearing plains';
      break;
    case 'CarbonRich':
      description = prng.choice([
        'Graphite-Carbide Crust, Diamond-Bearing Mantle Possible',
        'Dry Carbonaceous Highlands and Carbide Basins',
      ])!;
      break;
    case 'Chthonian':
      description = 'Exposed Iron-Silicate Core, Ablated Mantle Remnants';
      break;
    case 'Cryovolcanic':
      description = 'Fractured Ice Shell, Ammonia-Water Cryolava Flows';
      break;
    case 'DwarfIce':
      description = 'Low-Gravity Ice-Rock Regolith, Volatile Frost Fields';
      break;
    case 'Lunar':
      description = 'Impact-Pulverized Regolith, Basaltic Maria, Scarce Volatiles';
      break;
    case 'GasGiant':
      description = 'No Solid Surface Defined';
      break;
    case 'IceGiant':
      description = 'No Solid Surface Defined, Deep Icy/Fluid Mantle';
      break;
    case 'Frozen':
      description = prng.choice([
    ...
    default:
      ...
      logger.warn(`[SurfDesc] Unknown planet type '${planetType}' for lithosphere.`);
      break;
  }
