...
/** Generates gas giant physical base. */
function generateGasGiantPhysicalBase(prng: PRNG): { diameter: number; density: number } {
  ...
  logger.debug(
    `[PhysGen] Gas giant physical base: Diameter=${diameter}km, Density=${density.toFixed(2)} g/cm³`
  );
  return { diameter, density };
}

/** Generates ice giant physical base. */
function generateIceGiantPhysicalBase(prng: PRNG): { diameter: number; density: number } {
  ...
  logger.debug(
    `[PhysGen] Ice giant physical base: Diameter=${diameter}km, Density=${density.toFixed(2)} g/cm³`
  );
  return { diameter, density };
}

/** Generates solid physical base. */
function generateSolidPhysicalBase(prng: PRNG, planetType: string): { diameter: number; density: number } {
  ...
  const density = Math.max(
    ...
    Math.min(
      profile.maxDensity,
      profile.baseDensity + compressionBoost + prng.random(-profile.variance, profile.variance)
    )
  );

  logger.debug(
    `[PhysGen] ${planetType} physical base: Diameter=${diameter}km, Density=${density.toFixed(2)} g/cm³`
  );
  return { diameter, density };
}

/** Returns solid planet profile. */
