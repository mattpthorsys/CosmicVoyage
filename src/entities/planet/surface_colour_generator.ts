...
export function generateHeightLevelColors(planetType: string, rgbPalette: RgbColour[]): string[] | null {
  if (!rgbPalette || rgbPalette.length < 1) {
    logger.error(
      `[SurfColourGen:${planetType}] Cannot generate height colours: RGB palette cache is invalid or empty.`
    );
    return null;
  }
  ...
  logger.debug(
    `[SurfColourGen:${planetType}] Interpolating ${numPaletteColours} palette colours across ${CONFIG.PLANET_HEIGHT_LEVELS} height levels...`
  );
  ...
  for (let h = 0; h < CONFIG.PLANET_HEIGHT_LEVELS; h++) {
    ...
    if (index1 === index2 || factor === 0) {
      ...
      logger.warn(
        `[SurfColourGen:${planetType}] Height colour generation encountered invalid palette indices (${index1}, ${index2}) at height ${h}. Using fallback.`
      );
