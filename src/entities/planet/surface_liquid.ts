...
function getLiquidCoverage(
  ...
  if (hydro.includes('methane/ethane') || hydro.includes('hydrocarbon'))
    return surfaceTemp >= 88 && surfaceTemp <= 190 ? 0.12 : 0;
  if (hydro.includes('nitrogen frost plains') || hydro.includes('nitrogen/methane frosts'))
    return surfaceTemp >= 55 && surfaceTemp <= 115 ? 0.06 : 0;
  if (hydro.includes('ammonia-water') || hydro.includes('cryolava reservoirs'))
    return surfaceTemp >= 170 && surfaceTemp <= 260 ? 0.08 : 0;
  ...
  if ((hydro.includes('methane') || hydro.includes('nitrogen')) && surfaceTemp >= 65 && surfaceTemp <= 125)
    return 0.08;
  return 0;
}

/** Returns sea level for coverage. */
function getSeaLevelForCoverage(heightmap: number[][], coverage: number): number {
  ...
  const index = Math.max(
    0,
    Math.min(values.length - 1, Math.floor(values.length * Math.max(0.01, Math.min(0.96, coverage))))
  );
  return Math.round(values[index]);
}

/** Returns liquid kind. */
function getLiquidKind(
  planetType: string,
  hydrosphere: string,
  surfaceTemp: number
): SurfaceLiquidOverlay['kind'] {
  const hydro = hydrosphere.toLowerCase();
...
/** Returns liquid label. */
function getLiquidLabel(kind: SurfaceLiquidOverlay['kind']): string {
  switch (kind) {
    case 'hycean':
      return 'deep high-pressure ocean';
    case 'acid':
      return 'acidic liquid basin';
    case 'methane':
      return 'liquid methane basin';
    case 'nitrogen':
      return 'liquid nitrogen basin';
    case 'ammonia':
      return 'ammonia-water basin';
    case 'brine':
      return 'cold brine sea';
    case 'water':
    default:
      return 'liquid water ocean';
  }
}

/** Returns liquid colours. */
function getLiquidColours(kind: SurfaceLiquidOverlay['kind']): { colour: string; reflectiveColour: string } {
  switch (kind) {
    case 'hycean':
      return { colour: '#123B68', reflectiveColour: '#73BDE8' };
    case 'acid':
      return { colour: '#7B8E2F', reflectiveColour: '#D2E46D' };
    case 'methane':
      return { colour: '#1F6D82', reflectiveColour: '#8FD7EA' };
    case 'nitrogen':
      return { colour: '#547EA8', reflectiveColour: '#C0D8F0' };
    case 'ammonia':
      return { colour: '#4C9A9A', reflectiveColour: '#B4F0E8' };
    case 'brine':
      return { colour: '#1D5D7E', reflectiveColour: '#9BD8F4' };
    case 'water':
    default:
      return { colour: '#145F95', reflectiveColour: '#A8E8FF' };
  }
}
