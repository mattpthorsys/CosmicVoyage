...
const GAS_MOLECULAR_MASS_KG: Record<string, number> = {
  ...
  // Molecular masses are converted from atomic mass units to kilograms.
  ...
  Methanol: 5.32e-26, // CH₃OH (32.04 amu)
  ...
  'Sulfur Monoxide': 7.98e-26, // SO (48.06 amu)
...
function _getBasePrimaryWeights(
  ...
  if (planetType === 'Hycean') {
    ...
    return {
      'Carbon Dioxide': 72,
      Nitrogen: 15,
      'Sulfur Dioxide': 8 * metalVolatileFactor,
      'Water Vapor': 3,
      'Carbon Monoxide': 2,
    };
    ...
    return {
      'Carbon Monoxide': 28,
      'Silicon Monoxide': 24 * metalVolatileFactor,
      'Sulfur Dioxide': 18 * metalVolatileFactor,
      Nitrogen: 6,
    };
    ...
    return {
      'Carbon Dioxide': 32,
      'Carbon Monoxide': 24,
      Methane: approxTemp_K < 360 ? 18 * metalVolatileFactor : 4,
      Nitrogen: 18,
      'Hydrogen Cyanide': 6 * metalVolatileFactor,
    };
  } else if (planetType === 'Cryovolcanic' || planetType === 'DwarfIce') {
    return {
      Nitrogen: 36,
      Methane: 26 * metalVolatileFactor,
      Ammonia: 14 * metalVolatileFactor,
      Argon: 12,
      'Carbon Monoxide': 8,
    };
  ...
  } else if (approxTemp_K < 150) {
    // Cold
    return {
      Nitrogen: 48,
      Methane: 22 * metalVolatileFactor,
      'Carbon Dioxide': 14,
      Argon: 14,
      Ammonia: 8 * metalVolatileFactor,
    };
  } else if (approxTemp_K > 500) {
    // Hot
    return {
      'Carbon Dioxide': 48,
      Nitrogen: 18,
      'Sulfur Dioxide': 16 * metalVolatileFactor,
      'Water Vapor': 10,
      'Carbon Monoxide': 8,
    };
  } else {
    const oxygenChanceWeight =
      environment.ageGyr > 2.2 && approxTemp_K > 245 && approxTemp_K < 320 && activity < 1.6 ? 8 : 0.5;
...
/** Calculates the relative generation weight for a secondary atmospheric gas. */
function _secondaryGasWeight(
  gas: string,
  approxTemp_K: number,
  environment: StellarEnvironment,
  orbitAu: number
  ...
  if (['Oxygen', 'Ozone', 'Nitrous Oxide'].includes(gas)) {
    weight *=
      environment.ageGyr > 2.2 && approxTemp_K > 240 && approxTemp_K < 330 && activity < 1.7 ? 1.25 : 0.04;
  }
  if (['Chlorine', 'Fluorine', 'Hydrogen Chloride'].includes(gas)) weight *= approxTemp_K > 380 ? 0.8 : 0.12;
  if (['Hydrogen Cyanide', 'Acetylene', 'Diatomic Carbon'].includes(gas))
    weight *= environment.ageGyr < 1.5 || activity > 1.4 ? 1.5 : 0.55;
...
function _choosePrimaryGas(weights: Record<string, number>, totalWeight: number, prng: PRNG): string {
  ...
  if (totalWeight > 0) {
    ...
    logger.warn(
      `[AtmoGen] All potential primary gases likely escaped or weights were zero. Defaulting primary to Nitrogen.`
    );
  }
...
function _distributeSecondaryGases(
  ...
  logger.debug(
    `[AtmoGen] Available secondary gases after escape filter: ${availableGases.length > 0 ? availableGases.join(', ') : 'None'}`
  );

  for (let i = 1; i < numGasesToGenerate && remainingPercent > 0.1 && availableGases.length > 0; i++) {
    const totalGasWeight = availableGases.reduce(
      (total, gas) => total + _secondaryGasWeight(gas, approxTemp_K, environment, orbitAu),
      0
    );
    ...
    const percent =
      i === numGasesToGenerate - 1 || availableGases.length === 0
        ? remainingPercent
...
/** Estimates atmosphere temperature k. */
function estimateAtmosphereTemperatureK(
  planetType: string,
...
function _normalizeComposition(
  ...
  if (totalRaw > 0) {
    ...
    if (Math.abs(finalTotal - 100) > 0.1 && finalComp[primaryGas] !== undefined) {
      ...
      if (Math.abs(finalTotal - 100) > 0.1) {
        logger.warn(
          `[AtmoGen] Normalization adjustment failed to reach 100%. Final total: ${finalTotal.toFixed(1)}%`
        );
      }
      ...
      if (mostAbundantGas) {
        finalComp[mostAbundantGas] = Math.max(
          0,
          Math.round((finalComp[mostAbundantGas] + (100 - finalTotal)) * 10) / 10
        );
      }
    }
...
function generateAtmosphereComposition(
  ...
  logger.debug(
    `[AtmoGen] Generating composition for density '${density}' (V_esc: ${escapeVelocity.toFixed(0)} m/s)...`
  );
  if (density === 'None') {
...
export function generateAtmosphere(
  ...
  logger.debug(
    `[AtmoGen] Generating atmosphere (Type: ${planetType}, Gravity: ${gravity.toFixed(2)}g, V_esc: ${escapeVelocity.toFixed(0)} m/s)...`
  );
  ...
  if (densityRoll < 0.2)
    densityIndex = 0; // None
  else if (densityRoll < 0.5)
    densityIndex = 1; // Thin
  else if (densityRoll < 0.85)
    densityIndex = 2; // Earth-like
  ...
  if (planetType === 'GasGiant' || planetType === 'IceGiant' || planetType === 'Hycean') {
    ...
    if (escapeVelocity < earthLikeEscVel * 0.3 && densityIndex > 1) {
      // If V_esc < ~3360 m/s
    ...
    } else if (escapeVelocity < earthLikeEscVel * 0.7 && densityIndex > 2) {
      // If V_esc < ~7840 m/s
    ...
    if (environment.ageGyr > 7 && escapeVelocity < earthLikeEscVel * 0.8 && densityIndex > 0) {
      densityIndex -= 1;
      logger.debug(
        `[AtmoGen] Reduced density index to ${densityIndex} due to long-term atmospheric erosion.`
      );
    }
  ...
  let pressure =
    densityIndex === 0 ? 0 : Math.max(0.001, prng.random(0.01, 5) * (densityIndex + 1) * Math.sqrt(gravity));
  ...
  const composition = generateAtmosphereComposition(
    prng,
    finalDensity,
    planetType,
    escapeVelocity,
